...
usr <- "aragaki-kai"
pcr <- get_pcr("2022-05-16_uc6-src-ikd-w-wo-dox.xls", usr) |>
  read_pcr() |>
  scrub()

o <- pcr |>
  filter(!is.na(sample_name), target_name == "SRC")

ggplot(o, aes(sample_name, rq)) +
  geom_point()

pcr <- get_pcr("2022-06-03_uc6-nt-004.xls", usr) |>
  read_pcr() |>
  scrub()
...
ggsave("02_figures/kd-works-pcr.png", plot, width = 2, height = 2.5, dpi = 500)
